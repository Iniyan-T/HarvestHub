...
export function Messages() {
  ...
  const buyerIdFromUrl = searchParams.get("userId");
  const buyerNameFromUrl = searchParams.get("userName");
  const { userId: currentUserId } = getValidatedUserId(urlUserId);
  ...
  useEffect(() => {
    if (!buyerIdFromUrl || !currentUserId) return;
    const existing = conversations.find(
      (c) => c.otherUserId === buyerIdFromUrl
    );
    if (existing) {
      setSelectedConversation(existing._id);
      setSelectedOtherUserId(existing.otherUserId);
      setSelectedOtherName(
        existing.otherUser?.name || buyerNameFromUrl || "Buyer"
      );
    } else {
      setSelectedConversation("new_conversation");
      setSelectedOtherUserId(buyerIdFromUrl);
      setSelectedOtherName(
        buyerNameFromUrl ? decodeURIComponent(buyerNameFromUrl) : "Buyer"
      );
    }
  }, [buyerIdFromUrl, buyerNameFromUrl, currentUserId, conversations]);
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm
                  placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400
                  transition-all"
              />
            </div>
          </div>
                  ...
                  >
                    ...
                    <div className="relative flex-shrink-0">
                      <div
                        ...
                        ${
                          isActive
                            ? "bg-gradient-to-br from-emerald-500 to-emerald-700 shadow-md shadow-emerald-200"
                    ...
                    <div className="flex-1 min-w-0 text-left">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        {conv.unreadCount > 0 && (
                          <span className="flex-shrink-0 min-w-[18px] h-[18px] px-1 bg-emerald-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">
                            {conv.unreadCount > 99
                              ? "99+"
                              : conv.unreadCount}
                          </span>
        ...
        {selectedConversation ? (
          <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="bg-white border-b border-gray-200 px-4 lg:px-5 py-2.5 flex items-center gap-3 flex-shrink-0">
              ...
              <div className="relative flex-shrink-0">
                <div className="w-9 h-9 bg-gradient-to-br from-emerald-500 to-teal-500 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-emerald-600 font-medium flex items-center gap-1 leading-tight">
                            ...
                            >
                              <div
                                className={`max-w-[75%] lg:max-w-[65%] px-3 py-2 rounded-2xl
                                  ${
                                    isSender
                                      ? "bg-gradient-to-br from-emerald-500 to-emerald-600 text-white rounded-br-sm shadow-sm shadow-emerald-200/50"
            ...
            <div className="bg-white border-t border-gray-200 px-4 lg:px-5 py-2.5 flex-shrink-0">
              ...
              >
                ...
                <button
                  ...
                  className="w-9 h-9 flex-shrink-0 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white rounded-full
                    flex items-center justify-center hover:from-emerald-600 hover:to-emerald-700 active:scale-95
          ...
          <div className="hidden lg:flex flex-1 flex-col items-center justify-center bg-[#f0f2f5] min-w-0">
            <div className="text-center">
              <div className="w-20 h-20 bg-white rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-sm border border-gray-100 rotate-2">
                <MessageSquare className="w-10 h-10 text-emerald-300 -rotate-2" />
              </div>
              <p className="text-lg font-semibold text-gray-700">
                Select a conversation
              </p>
              <p className="text-sm text-gray-400 mt-1.5 max-w-[240px] mx-auto leading-relaxed">
                Choose a buyer from the list to view your messages
              </p>
            </div>
          </div>
        )}
